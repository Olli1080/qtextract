...
struct SignatureDefinitionElf {
    id: i32,
    x64: bool,
    signature: &'static [(u8, bool)],
    extractor: fn(offset: usize, bytes: &[u8], elf: &Elf) -> Option<QtResourceInfo>
}
...
fn x64_extract_elf(bytes_offset: usize, bytes: &[u8], elf: &Elf) -> Option<QtResourceInfo> {
    ...
    for i in 0..3 {
        stream.skip(3);
        let v = stream.read_u32::<false>()? as usize;
        result[i] = bytes_offset + stream.position() + v;
    }
...
static TEXT_SIGNATURES_PE: &[SignatureDefinition] = &[
    SignatureDefinition {
        /*
...
static TEXT_SIGNATURES_ELF: &[SignatureDefinitionElf] = &[
...
fn get_target_section_elf<'a>(elf: &'a Elf) -> Option<&'a SectionHeader> {
    if !check_opt("--scanall") {
        if let Some(target) = check_opt_arg("--section") {
            for v in &elf.section_headers {
                let name = elf.shdr_strtab.get_at(v.sh_name).unwrap();
                if name == target {
                    return Some(v);
                }
            }
...
fn main() {
    ...
    fn get_ressources(buffer: &Vec<u8>) -> error::Result<Option<Vec<QtResourceInfo>>> {
